...
/*	public void findIC(){
		...
		int[] ic_number = new int[components];
		for(i = 0; i < components; i++)
			ic_number[i] = 0;
		for(k = 0; k < components; k++){
			for(i = 1; i <= dimensions[k]; i++)  { 
				int[] array = new int[dimensions[k]];
				ic_number[i] = ic_number[i] +	combinations(0,dimensions[k], i, k, array, i, 0);
	...
	// 1 2 
	public int combinations(int start, int end, int number, int k, int[] arr, int n, int index){
		...
		int i, j = 0;
		for(i = 0; i <= end; i++)
			combinations(start+1, end, number-1, k, arr, n, index);
